...
파일: `src/components/InputBox.tsx`

1. 사용자가 입력 카드의 마이크 버튼을 누른다.
2. 브라우저가 Web Speech API를 지원하면 한국어 음성 인식을 시작한다.
3. 중간 인식 결과는 안내 영역에 표시된다.
4. 확정된 문장은 기존 입력창에 추가된다.
5. 사용자는 기존과 동일하게 분류 버튼을 눌러 AI 분류를 실행한다.
6. 브라우저가 음성 인식을 지원하지 않거나 마이크 권한이 없으면 안내 문구를 표시한다.

`/?voice=1`로 진입하면 음성 입력 안내가 활성화된다. iPhone 단축어, 동작 버튼, 뒷면 탭에서 이 URL을 열도록 연결하면 앱 진입까지의 동작을 줄일 수 있다.
...
현재 PWA 설치 준비는 manifest와 아이콘 연결까지 진행되어 있다. 오프라인 캐싱과 Web Push용 service worker는 아직 구현하지 않았다.
